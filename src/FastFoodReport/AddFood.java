...
public class AddFood extends javax.swing.JFrame {
    ...
    private void Getdata(){
        ...
        for(int i=0;i<List_FoodStock.list_foodstock.size();i++){
            Object row[]={
                List_FoodStock.list_foodstock.get(i).getCode(),
                List_FoodStock.list_foodstock.get(i).getName(),
                List_FoodStock.list_foodstock.get(i).getPrice()+"$",
                List_FoodStock.list_foodstock.get(i).getImage(),
            };
            model.addRow(row);
        }
    ...
    private void btnremoveActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnremoveActionPerformed
        if(JOptionPane.showConfirmDialog(null, "Do you want to remove ?", "FOOD DRINK", JOptionPane.YES_NO_OPTION)==JOptionPane.YES_NO_OPTION){
            db.RemoveFoodStock(Integer.parseInt(txtcode.getText()));
        }
        Clear();
        Getdata();
